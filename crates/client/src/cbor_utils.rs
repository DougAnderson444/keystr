...
pub fn extract_p256_public_key_from_attestation(
    attestation_object: &[u8],
) -> Result<Vec<u8>, String> {
    // Parse CBOR attestation object
    let attestation: CborValue = ciborium::from_reader(attestation_object)
        .map_err(|e| format!("Failed to parse attestation object: {}", e))?;

    // Get authData field from the map
    let auth_data = match &attestation {
        CborValue::Map(map) => {
            // Find the authData key in the map
            let auth_data_value = map
                .iter()
                .find(|(k, _)| matches!(k, CborValue::Text(t) if t == "authData"))
                .map(|(_, v)| v)
                .ok_or_else(|| "Missing authData in attestation object".to_string())?;

            match auth_data_value {
                CborValue::Bytes(data) => data,
                _ => return Err("Invalid authData type in attestation object".to_string()),
            }
        }
...
fn extract_public_key_from_auth_data(auth_data: &[u8]) -> Result<Vec<u8>, String> {
    ...
    // Parse COSE key
    let cose_key: CborValue = ciborium::from_reader(cose_key_bytes)
...
fn extract_p256_from_cose_key(cose_key: &CborValue) -> Result<Vec<u8>, String> {
    ...
    let _kty = map
        .iter()
        .find(|(k, _)| matches!(k, CborValue::Integer(i) if *i == 1.into()))
        .and_then(|(_, v)| match v {
            CborValue::Integer(i) if *i == 2.into() => Some(2),
            _ => None,
        })
        .ok_or_else(|| "Invalid or missing key type (kty)".to_string())?;

    // Check algorithm (alg = 3, should be -7 for ES256/P-256)
    let _alg = map
        .iter()
        .find(|(k, _)| matches!(k, CborValue::Integer(i) if *i == 3.into()))
        .and_then(|(_, v)| match v {
            CborValue::Integer(i) if *i == (-7).into() => Some(-7),
            _ => None,
        })
        .ok_or_else(|| "Invalid or missing algorithm (alg), expected -7 for ES256".to_string())?;
    ...
    let _crv = map
        ...
        .find(|(k, _)| matches!(k, CborValue::Integer(i) if *i == (-1).into()))
        .and_then(|(_, v)| match v {
            CborValue::Integer(i) if *i == 1.into() => Some(1),
            _ => None,
        })
        .ok_or_else(|| "Invalid or missing curve (crv), expected 1 for P-256".to_string())?;

    // Extract x coordinate (key = -2)
    let x = map
        .iter()
        .find(|(k, _)| matches!(k, CborValue::Integer(i) if *i == (-2).into()))
        .and_then(|(_, v)| match v {
    ...
    if x.len() != 32 {
        return Err(format!("Invalid x coordinate length: {}", x.len()));
    }
    ...
    if y.len() != 32 {
        return Err(format!("Invalid y coordinate length: {}", y.len()));
    }
...
mod tests {
    ...
    fn test_der_signature_parsing() {
        ...
        let der_sig = vec![
            ...
            0x1d, 0x1e, 0x1f, 0x20, 0x02, 0x20, // INTEGER, 32 bytes (s)
